...
document.getElementById('title').innerHTML = user + "@mail.haxmail.buzz";
...
const url = 'https://haxmaildb-lioncat2002.turso.io';
req.open("POST", url);
const readonly_token = "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicm8iLCJpYXQiOjE2ODQ2NjI4OTEsImlkIjoiY2M3MjIzMTMtZjZlNC0xMWVkLWJlNzMtZTZhNjIxNDQ3ZDlmIn0.hTvrRgqj8URjqpVVEAgBnYSbvmgWzQ9uugWL761PfZH5lghEgPn0UAe-60kw2b2971ZmxNu7Vso3exMEYP4nCw";
req.setRequestHeader('Authorization', 'Bearer ' + readonly_token)

req.send(JSON.stringify({ statements: [{ q: "SELECT date, sender, recipients, data FROM mail WHERE recipients = ? ORDER BY ROWID DESC LIMIT ? OFFSET ?", params: ["<" + user + "@mail.haxmail.buzz>", PAGE_SIZE, offset] }] }));
...
function createTable(data) {
    ...
    if (data.rows.length == 0) {
        ...
        h.innerHTML = "No e-mails for " + user + "@mail.haxmail.buzz yet! <br> &#8635; refresh";
